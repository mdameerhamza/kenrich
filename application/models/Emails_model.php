...
class Emails_model extends CRM_Model
{
    ...
    public function send_email_template($template_slug, $email, $merge_fields, $ticketid = '', $cc = '')
    {
        ...
        */
        ...
        $this->mail->setFrom(($from_header_dept_email ? $ticket->department_email : $fromemail), $fromname);
        ...
        if ($reply_to != false) {
            ...
            $this->mail->addReplyTo($template->reply_to);
        }
        ...
        if ($template->plaintext == 0) {
            ...
            $this->email->set_alt_message(trim($alt_message));
             ...
             $this->mail->AltBody = trim($alt_message);
        }
        ...
        if (count($attachments) > 0) {
            foreach ($attachments as $attach) {
                if (!isset($attach['read'])) {
                    $this->email->attach($attach['attachment'], 'attachment', $attach['filename'], $attach['type']);
                    $this->mail->addAttachment($attach['attachment'], 'attachment', $attach['filename'], $attach['type']);   
                } else {
                    $this->email->attach($attach['attachment'], '', $attach['filename']);
                    $this->mail->addAttachment($attach['attachment'], '', $attach['filename']); 
                }
            }
        }
        ...
        if ($this->email->send() || $this->mail->send()) {
